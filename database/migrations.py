...
_NEW_COLUMNS = {
    "chat_settings": {
        ...
        "ref_link": "TEXT",
        # Тип чата ('group'/'supergroup'/'channel'/''=ещё не определён). Нужен,
        # чтобы НИКОГДА не слать рассылки/рекламу в каналы (бот может быть там
        # админом только как цель задания-подписки, постить туда нельзя).
        ...
        # Мягкая модерация: сколько ПЕРВЫХ нарушений охотника караются только
        # удалением + вежливой просьбой (без счётчика к муту/бану). По умолчанию
        # бот «солдат, а не палач»: сначала по-доброму, наказания — в крайнем случае.
        "soft_grace": "INTEGER DEFAULT 2",
